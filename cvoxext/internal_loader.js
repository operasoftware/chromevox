...
var loadScript = function(url) {
  ...
  else if (urlStartsWith('http://b/') ||
           urlStartsWith('http://buganizer/') ||
           urlStartsWith('http://b.corp.google.com/') ||
           urlStartsWith('http://buganizer.corp.google.com/')) {
    scriptName = 'buganizer';
  }
  ...
  if (scriptName) {
    console.log('Loading extension ' + scriptName);
...
/**load appropriate version of ChromeVox, can load Web Store, ChromeOS,
  uncompiled and dev versions */
var checkChromeVoxAndLoad = function() {
  ...
  var chromeVoxIDs = {
    ...
    local: 'jpakpggombnoldlomllehhiifohacocg',
    develop: 'bdcfgfeioooifpgmbfjpopbcbgdmehnb'
  };

  //try loading development version
  var port = chrome.extension.connect(chromeVoxIDs.develop);
  window.setTimeout(
    function() {
      try {
        port.postMessage();
        ...
        //try loading Web Store Version
        var port2 = chrome.extension.connect(chromeVoxIDs.webstore);
        window.setTimeout(function() {
          try {
            port2.postMessage();
            loadScript(document.location.href);
          } catch (e) {
            //try loading ChromeOS version
            var port3 = chrome.extension.connect(chromeVoxIDs.chromeos);
            window.setTimeout(function() {
              try {
                port3.postMessage();
                loadScript(document.location.href);
              } catch (e) {
                //try loading uncompiled version
                var port4 = chrome.extension.connect(chromeVoxIDs.local);
                window.setTimeout(function() {
                  try {
                    port4.postMessage();
                    loadScript(document.location.href);
                    ...
                    var install = confirm(
                      'ChromeVox Extensions needs ChromeVox. ' +
