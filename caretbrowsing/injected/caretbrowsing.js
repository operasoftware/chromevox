 ...
 *
 * Some details about how WebKit selection works, which will help in
 ...
 *
 * Finally, WebKit will throw an exception if you try to set an invalid
 ...
 * WebKit selection wherever possible, but displays its own flashing
...
CaretBrowsing.isControlThatNeedsArrowKeys = function(node) {
  ...
  if (node.getAttribute && cvox.DomUtil.isFocusable(node)) {
...
CaretBrowsing.setInitialCursor = function() {
  ...
  var start = new cvox.Cursor(document.body, 0, '');
  var end = new cvox.Cursor(document.body, 0, '');
  var elementsEntered = [];
  var elementsLeft = [];
 ...
 * animated from a larger box to the final cursor location to
 * make it easier for the user to find when it first appears.
 */
CaretBrowsing.createCaretElement = function() {
  ...
  element.style.borderLeft = '1px solid #fff';
  element.style.minHeight = '10px';
  var width = CaretBrowsing.caretWidth;
  var height = CaretBrowsing.caretHeight;

  // The property that will animate. This is the initial state,
  // just shown transiently.
  ...
  // Tell TraverseUtil not to traverse this node.
  ...
  window.setTimeout(function() {
    // If we have a cursor, this animates into the cursor. If it's a selection,
    // the animation disappears as it becomes the selection.
 ...
 * @param {cvox.Cursor} cursor A single cursor position.
...
CaretBrowsing.updateCaretOrSelection = function(scrollToSelection) {
  ...
  if (CaretBrowsing.isControlThatNeedsArrowKeys(document.activeElement)) {
    ...
    var rect = CaretBrowsing.getCursorRect(
        new cvox.Cursor(range.startContainer,
                        range.startOffset,
  ...
  if (scrollToSelection) {
    ...
    var rect = CaretBrowsing.getCursorRect(
        new cvox.Cursor(sel.focusNode, sel.focusOffset,
                        cvox.TraverseUtil.getNodeText(sel.focusNode)));
 ...
 * @param {Selection} sel The selection.
 * @return {cvox.Cursor} A cursor pointing to the selection's anchor location.
 */
CaretBrowsing.makeAnchorCursor = function(sel) {
  return new cvox.Cursor(sel.anchorNode, sel.anchorOffset,
                         cvox.TraverseUtil.getNodeText(sel.anchorNode));
};

/**
 * Create a Cursor from the focus position of the selection.
 * @param {Selection} sel The selection.
 * @return {cvox.Cursor} A cursor pointing to the selection's focus location.
 */
CaretBrowsing.makeFocusCursor = function(sel) {
  return new cvox.Cursor(sel.focusNode, sel.focusOffset,
                         cvox.TraverseUtil.getNodeText(sel.focusNode));
};
 ...
 * @param {Selection} sel The selection.
 * @return {cvox.Cursor} A cursor pointing to the selection's left boundary.
 */
 ...
 * @param {Selection} sel The selection.
 * @return {cvox.Cursor} A cursor pointing to the selection's right boundary.
 */
 ...
 * @param {cvox.Cursor} start The start position.
 * @param {cvox.Cursor} end The end position.
...
/**
 * Note: the built-in WebKit function by the same name is unreliable.
 * @param {Selection} sel The selection.
 ...
 * @param {cvox.Cursor} cursor The current cursor location.
 ...
 */
CaretBrowsing.forwards = function(cursor, elementsEntered, elementsLeft) {
  var previousCursor = cursor.clone();
  var result = cvox.TraverseUtil.forwardsChar(
 ...
 *     document has been reached.
 */
CaretBrowsing.backwards = function(cursor, elementsEntered, elementsLeft) {
  var previousCursor = cursor.clone();
  var result = cvox.TraverseUtil.backwardsChar(
      cursor, elementsEntered, elementsLeft);
...
CaretBrowsing.moveRight = function(evt) {
  ...
  while (true) {
    var result;
    if (evt.ctrlKey) {
      result = cvox.TraverseUtil.getNextWord(
          previousEnd, end, elementsEntered, elementsLeft);
    } else {
      previousEnd = end.clone();
      result = CaretBrowsing.forwards(end, elementsEntered, elementsLeft);
    }
  ...
  if (!evt.shiftKey) {
    elementsEntered.push(end.node);
    CaretBrowsing.setFocusToFirstFocusable(elementsEntered);
  }
...
CaretBrowsing.moveLeft = function(evt) {
  ...
  while (true) {
    var result;
    if (evt.ctrlKey) {
      result = cvox.TraverseUtil.getPreviousWord(
          start, previousStart, elementsEntered, elementsLeft);
    } else {
      previousStart = start.clone();
      result = CaretBrowsing.backwards(start, elementsEntered, elementsLeft);
    }
  ...
  if (!evt.shiftKey) {
    elementsEntered.push(start.node);
    CaretBrowsing.setFocusToFirstFocusable(elementsEntered);
  }
...
CaretBrowsing.moveDown = function(evt) {
  ...
  var y = -1;
  while (true) {
    if (null === CaretBrowsing.forwards(
        rightPos, elementsEntered, elementsLeft)) {
...
CaretBrowsing.moveUp = function(evt) {
  ...
  var y = 999999;
  while (true) {
    if (null === CaretBrowsing.backwards(
        leftPos, elementsEntered, elementsLeft)) {
      CaretBrowsing.setAndValidateSelection(
...
CaretBrowsing.escapeFromControl = function(control) {
  ...
  var start = new cvox.Cursor(control, 0, '');
  var previousStart = start.clone();
  var end = new cvox.Cursor(control, 0, '');
  var previousEnd = end.clone();
  ...
  while (true) {
    if (null === CaretBrowsing.backwards(
        start, elementsEntered, elementsLeft)) {
      break;
    }
  ...
  while (true) {
    if (null === CaretBrowsing.forwards(end, elementsEntered, elementsLeft)) {
      break;
    }
    if (cvox.DomUtil.isDescendantOfNode(end.node, control)) {
      previousEnd = end.clone();
      continue;
    }
  ...
  if (!cvox.DomUtil.isDescendantOfNode(previousStart.node, control)) {
    start = previousStart.clone();
  }

  if (!cvox.DomUtil.isDescendantOfNode(previousEnd.node, control)) {
...
/**
 * Event handler, called when the mouse is clicked. WebKit already
...
CaretBrowsing.caretBlinkFunction = function() {
  if (CaretBrowsing.caretElement) {
    if (CaretBrowsing.blinkFlag) {
      ...
      CaretBrowsing.caretElement.style.visibility = 'hidden';
      CaretBrowsing.blinkFlag = true;
    }
  }
};
...
/**
 * Called when we get a message from the background page updating the
 * enabled/disabled state of caret browsing.
