...
if (!COMPILED) {
  ...
  goog.writeScriptTag_ = function(src) {
    ...
    if (goog.global.CHROME_CONTENT_SCRIPT) {
      function loadNextScript() {
        if (goog.queue_.length == 0)
          return;
        var doc = goog.global.document;
        var scriptElt = document.createElement('script');
        scriptElt.type = 'text/javascript';
        scriptElt.src = goog.queue_[0] + '?' + new Date().getTime();
        doc.getElementsByTagName('head')[0].appendChild(scriptElt);
        scriptElt.onload = function() {
          goog.queue_ = goog.queue_.slice(1);
          loadNextScript();
        };
      }

      goog.queue_.push(src);
      if (goog.queue_.length == 1) {
        loadNextScript();
      }
    } else {
      var doc = goog.global.document;
